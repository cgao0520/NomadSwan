...
//#include <io.h>
//#include <fcntl.h>
#include "oswan.h" // just for Macro WS_SYSTEM_MONO and WS_SYSTEM_COLOR
#include "ws.h"
#include "log.h"
#include "rom.h"
...
//#include "Jajamaru_ws.h"
//#include "Dicing_wsc.h"
//#include "Seiya_wsc.h"
//#include "RockmanE_wsc.h"
//#include "Robot_ws.h"
...
//  BYTE* rom = ws_rom_load_zip(path);
  BYTE* rom = NULL;
/* 
  		romSize = Robot_ws_size;     
			rom = (BYTE*) malloc(romSize); 
			memcpy(rom,Robot_ws,romSize);
*/			
			
		FileHandle* file = file_open(path, "r");
    //char line[100];
    ...
    {
    	romSize = file_getSize(file);
    	rom = (BYTE*)malloc(romSize);
    	//void file_read(void* dest, int bs, int size, FileHandle* fileHandle) 
    	file_read(rom,1,romSize,file);
    	file_close(file);
    }			
  
/*  srand(WsPlatformGetTime());
  int k=rand()%(4+1);
  switch(k)
  {
  	case 0:
  		romSize = Jajamaru_ws_size;     
			rom = (BYTE*) malloc(romSize);  
			memcpy(rom,Jajamaru_ws,romSize);
			break;
		case 1:
			romSize = Dicing_wsc_size;     
			rom = (BYTE*) malloc(romSize); 
			memcpy(rom,Dicing_wsc,romSize);
			break;
		case 2:
			romSize = Seiya_wsc_size;
  		rom = (BYTE*) malloc(romSize);
  		memcpy(rom,Seiya_wsc,romSize);    
  		break;
  	default:
			romSize = RockmanE_wsc_size;
  		rom = (BYTE*) malloc(romSize);
  		memcpy(rom,RockmanE_wsc,romSize);   		
  		break;
  }*/
