...
void WsResize(void)
{
	//This function can be modified and used to fit the 3DS when you want to play vertical game in horizontal 3DS.
  //WsDrawRelease(); //c_gao
  //WsDrawCreate(hWnd); //c_gao
  ...
  //  client_width  = lcdHeight * DrawSize; //c_gao
  //  client_height = lcdWidth  * DrawSize + barHeight; //c_gao
...
void WsPause(void)
{
  //HMENU	menu = GetMenu(hWnd);//c_gao

  if (app_gameRunning) {
    ws_audio_stop();
    app_gameRunning = 0;
    //CheckMenuItem(menu, ID_FILE_PAUSE,MF_CHECKED);//c_gao
    //oswan_set_status_text(0, "Pause");//c_gao
  }
  else if (ws_rom_path[0]) {
    app_gameRunning = 1;
    //CheckMenuItem(menu,ID_FILE_PAUSE,MF_UNCHECKED);//c_gao
  }
}
...
void WsReset(void)
{
  if (ws_rom_path[0]) {
    ws_reset();
    //oswan_set_status_text(1, "Reset");//c_gao
  }
}
...
void oswan_open_rom(const char* path)
{
  ...
  //GetBaseName(ws_rom_path);//this function has some problems
}
...
int GetKeyState()
{
	...
	if(keys & KEY_R)
	{
		//app_new_rom = 0;
		//app_gameRunning = 0;		
		char* file = startFileChooser(extentions,true);
...
int main(void) 
{
		...
		memset(fbt, 0, 240*400*3);
		int offset = (88*240+48+144)*3;
		CRGBA text_clr(0,255,0,0);
		CRGBA bk_clr(0,0,0,0);
	/*	
		//bottom screen
		u8* fbb = gfxGetFramebuffer(GFX_BOTTOM, GFX_LEFT, NULL, NULL);
		//memset(fbb,0,240*320*3);
		CFont8 font(fbb,text_clr,bk_clr);
		font.DrawString(100,0,"\t\tNomadSwan, a WonderSwan emulator.\n\t\tMade by Vincent(c_gao). \
		\n\t\tBLOG: http://blog.congao.net\n\t\te-mail: dr.c.gao@gmail.com");
		*/
/*
		printf("\tNomadSwan, a WonderSwan emulator.\r\n");
		printf("\tMade by Vincent(c_gao).\r\n\r\n");
		printf("\tBLOG: http://blog.congao.net\r\n");
		printf("\te-mail: dr.c.gao@gmail.com");
		
		FileHandle* file = file_open("/test.txt", "r");
    char line[100];
    ...
    {
    	int flen = file_getSize(file);
    	char* fcont = (char*)malloc(flen+1);
    	//void file_read(void* dest, int bs, int size, FileHandle* fileHandle) 
    	file_read(fcont,1,flen,file);
    	fcont[flen]=0;
    	printf(fcont);
    	free(fcont);
    }	
...
*/		
    ...
    strcat(cwd,file);
			...
			//clearConsole();
			//iprintf("Loading:%s, wait 3 Secs.\n",cwd);
			//consoleFlush();
			//Sleep(3000);
			oswan_open_rom(cwd);
			free(file);
			
			//iprintf("open_file OK.\n");
			//consoleFlush();
			//Sleep(3000);			
		}		
	...
	while (aptMainLoop())
	{
		...
		// Example rendering code that displays a white pixel
		// Please note that the 3DS screens are sideways (thus 240x400 and 240x320)
//		fbt = gfxGetFramebuffer(GFX_TOP, GFX_LEFT, NULL, NULL);
//		fbt += offset;
...
/*		font.DrawString(100,0,"\t\tNomadSwan, a WonderSwan emulator.\n\t\tMade by Vincent(c_gao). \
		...
		static char sfm[10];
		memset(sfm,0,10);
		sprintf(sfm,"frame=%d",i++);
		font.DrawString(200,0,sfm);
*/
//		font.DrawString(10,10,statuf_str);		
