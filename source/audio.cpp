...
//
/////////////////////////////////////////////////////////////////////////////////////////////////
void ws_audio_device_init()
{
  for (int i = 0; i < BUFCNT; i++)
    //waveData[i] = (short*) malloc(byte_buffer_size);//c_gao
    waveData[i] = (short*)linearAlloc(byte_buffer_size);

/*  if (WsDirectSound)
    WsDirectSoundCreate();
  else*/
  
  	CSND_initialize(NULL);//c_gao
...
/*
  ...
  for (int i = 0; i < BUFCNT; i++)
    if (waveData[i]) {
      //free(waveData[i]);//c_gao
      linearFree(waveData[i]);
      waveData[i] = NULL;
    }
...
void WsWaveSet(BYTE voice, BYTE hvoice_l, BYTE hvoice_r)
{
  ...
  int    index;
  short  value, lVol[5], rVol[5];
  short  LL, RR, VV, HVL, HVR;
  ...
  if (channel_is_playing[5]) {
    ...
    preindex[4] = index;
    point[4]++;
    value = ((short) PDataN[NoisePattern][index]) - 8;
  ...
  waveData[wBuf][wPos++] = LL * MainVol / 3;
  waveData[wBuf][wPos++] = RR * MainVol / 3;
...
//CSND_playsound(8+playingBuffer, 0, CSND_ENCODING_PCM16, CSND_FREQUENCY, addr, addr, CSND_BUFFER_SIZE*2, 0, 0);
	CSND_playsound(8+rBuf, CSND_LOOP_DISABLE , CSND_ENCODING_PCM16, bps, (u32*) (waveData[rBuf]), (u32*) (waveData[rBuf]), byte_buffer_size, 0, 0);
  rBuf = (rBuf + 1) % BUFCNT;
}
/*
  for (int i = 0; i < BUFCNT; i++) {
