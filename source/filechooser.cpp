...
FileChooserState romChooserState = {0,"/ws"};
...
char* startFileChooser(const char* extensions[], bool romExtensions, bool canQuit) 
{
    ...
    //setPrintConsole(menuConsole);//c_gao
    fileChooserOn = true;
    //updateScreens(true); // Screen may need to be enabled//c_gao

    int numExtensions = sizeof(extensions)/sizeof(const char*);
    char* retval;// = ws_rom_path; memset(retval, 0, 1024);
        ...
        while ((entry = fs_readdir()) != NULL) 
        {
            ...
            if (!(entry->d_type & DT_DIR)) // folder
            {
                if (ext) 
                {
                    ...
                    if (romExtensions) 
                    {
                        isRomFile = strcasecmp(ext, "ws") == 0 || strcasecmp(ext, "wsc") == 0;
                        if (isRomFile)
                            isValidExtension = true;
                    }
                }
            ...
            if (entry->d_type & DT_DIR || isValidExtension)  // folder or rom file
            {
...
/*  //not needed for now     
            else if (ext && strcasecmp(ext, "yss") == 0 && !(entry->d_type & DT_DIR)) // it is a file and has extension of "yss", it is a suspend file
            {
                bool matched = false;
...
*/            
        ...
        while (!readDirectory) 
        {
            ...
            buffer[screenLen] = '\0';
            iprintf("%s", buffer);//iprintfColored(CONSOLE_COLOR_WHITE, "%s", buffer);
            for (uint j=0; j<screenLen-strlen(buffer); j++)
                iprintf(" ");//iprintfColored(CONSOLE_COLOR_WHITE, " ");
            ...
            for (int i=scrollY; i<scrollY+filesPerPage && i<numFiles; i++) 
            {
                ...
                buffer[stringLen] = '\0';
                if (flags[i] & FLAG_DIRECTORY) {
                    iprintf("%s/", buffer);//iprintfColored(CONSOLE_COLOR_LIGHT_YELLOW, "%s/", buffer);
                }
                else if (flags[i] & FLAG_SUSPENDED)
                    iprintf("%s", buffer);//iprintfColored(CONSOLE_COLOR_LIGHT_MAGENTA, "%s", buffer);
                else
                    iprintf("%s", buffer);//iprintfColored(CONSOLE_COLOR_WHITE, "%s", buffer);
...
*/
            ...
            if (canQuit) 
            {
                ...
                iprintf("                Press Y to exit");//iprintfColored(CONSOLE_COLOR_WHITE, "                Press Y to exit");
            }
