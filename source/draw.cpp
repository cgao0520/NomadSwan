...
#include <stdio.h>
#include <stdlib.h>
#include "oswan.h" //for some variables and Macros
#include "draw.h"
#include "segment.h"
#include "segmentM.h"
#include "io.h" //ws_ioRam used
//#include "log.h"
#include "gpu.h"
...
/*	if(DrawMode & 1)
	...
	//}//c_gao
	fclose(fp);
    //oswan_set_status_text(1, "Screenshot %d", filecount);//c_gao
}
////////////////////////////////////////////////////////////////////////////////
//
////////////////////////////////////////////////////////////////////////////////
...
//added by c_gao (the following function)
void WsDrawResize()
{
#ifdef PLATFORM_WIN32
	g_dib.Create(224,-144,32,DIB_PIXEL_FORMAT32_RGB); //Create function will automatically delete the previous dib
	
	if (ws_system ==  WS_SYSTEM_COLOR) // WSC
		g_dib_seg.Create(8,-144,32,DIB_PIXEL_FORMAT32_RGB);
	else //WS Mono
		g_dib_seg.Create(224,-8,32,DIB_PIXEL_FORMAT32_RGB);
#endif
	...
	if(g_dib_seg)
		free(g_dib_seg);
	if (ws_system ==  WS_SYSTEM_COLOR) // WSC
		g_dib_seg = (u8*)malloc(144*8*sizeof(DWORD));
	else //WS Mono
		g_dib_seg = (u8*)malloc(224*8*sizeof(DWORD));
#endif
}
////////////////////////////////////////////////////////////////////////////////
//
...
////////////////////////////////////////////////////////////////////////////////
/*
//commented but may useful in the future
...
{
#ifdef PLATFORM_WIN32
	memcpy(g_dib.GetBits(),backbuffer,224*144*4);
	HDC hdc = ::GetDC(hWnd);
	g_dib.BitBlt(hdc,0,0,224,144,0,0);
	ReleaseDC(hWnd,hdc);
#endif
	...
	for(i=0; i<144; i++)
	{
		for(j=0; j<224; j++)
		{
			...
			g=(clr>>8)&0xff;
			b=clr&0xff;
			putpixel_FB(i,j,framebuffer,r,g,b);
		}
	}
#endif
}
...
#ifdef PLATFORM_WIN32	
	if (ws_system ==  WS_SYSTEM_COLOR) // WSC
		memset(g_dib_seg.GetBits(),0,8*144*sizeof(DWORD));//added by c_gao
	else //WS Mono
		memset(g_dib_seg.GetBits(),0xff,224*8*sizeof(DWORD));//added by c_gao
#endif
#ifdef PLATFORM_3DS	
	if (ws_system ==  WS_SYSTEM_COLOR) // WSC
		memset(g_dib_seg,0,8*144*sizeof(DWORD));//added by c_gao
	else //WS Mono
		memset(g_dib_seg,0xff,224*8*sizeof(DWORD));//added by c_gao
#endif
//	HRESULT			hRet;
//	DDSURFACEDESC2	ddsd;
	RECT			rect;
	...
	for (int disp = 0; disp < 6; disp++)
	{
		if (lcd & 1)
		{
			if (ws_system ==  WS_SYSTEM_COLOR) // WSC
			{
				...
				pos >>= 1;//added by c_gao
			/*	if(DrawMode & 0x01) //vertical mode
				{
					rect.left = pos;
					rect.top = 0;
					rect.right = pos + LCD_SEGMENT_WIDTH;//pos + LCD_SEGMENT_WIDTH * 2;
					rect.bottom = LCD_SEGMENT_WIDTH;//LCD_SEGMENT_WIDTH * 2;
				}
				else //horizontal mode
				{*/
					rect.left = 0;
					rect.top = pos;
					rect.right = LCD_SEGMENT_WIDTH;//LCD_SEGMENT_WIDTH * 2;
					rect.bottom = pos + LCD_SEGMENT_WIDTH;//pos + LCD_SEGMENT_WIDTH * 2;
				//}

				//start added by c_gao
				unsigned char* pbuf = g_dib_seg;
				pbuf += rect.top*8*sizeof(DWORD);
				memcpy(pbuf,psrcd,8*8*sizeof(DWORD));
				//end added by c_gao
			}
			else // WS Mono
			{
			...
			/*	if(DrawMode & 0x01)
				{
					rect.left = 0;
					rect.top = 224-LCD_SEGMENT_WIDTH-pos;//448-(LCD_SEGMENT_WIDTH * 2)-pos;
					rect.right = LCD_SEGMENT_WIDTH;//LCD_SEGMENT_WIDTH * 2;
					rect.bottom = 224-pos;//448-pos;
				}
				else
				{*/
					rect.left = pos;
				...
				for(int i=0; i<8; i++)
				{
					memcpy(pbuf+i*224*sizeof(DWORD)+rect.left*sizeof(DWORD), psrcd+i*8/*psrcd is already DWORD point*/, 8*sizeof(DWORD));
				}
				//end added by c_gao
			}
...
/*
	...
	HDC hdc = ::GetDC(hWnd);
	if (ws_system ==  WS_SYSTEM_COLOR) // WSC
		g_dib_seg.BitBlt(hdc,224,0,8,144,0,0);
	else // WS Mono
		g_dib_seg.BitBlt(hdc,0,144,244,8,0,0);
	ReleaseDC(hWnd,hdc);
	//end added by c_gao
#endif
#ifdef PLATFORM_3DS
